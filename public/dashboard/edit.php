...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $id = $_POST['id'];
  // Insert player data
  $sql_player = "UPDATE player SET first_name=?, last_name=?, position=?, nationalite_id=?, league_id=?, club_id=? WHERE player_id=$id";
  ...
  if (mysqli_stmt_execute($stmt_player)) {
    $player_id = mysqli_insert_id($conn); // Get inserted player ID for stats linkage
  } else {
  ...
  // Insert stats data based on position
  if ($position == 'GK') {
    $diving = $_POST['diving'];
    ...
    $sql_stats = "UPDATE goalkeeper_stats SET rating=?, diving=?, handling=?, kicking=?, positioning=?, reflexes=?, speed=? WHERE player_id=$id";
    $stmt_stats = mysqli_prepare($conn, $sql_stats);
    mysqli_stmt_bind_param($stmt_stats, 'iiiiiii', $rating, $diving, $handling, $kicking, $positioning, $reflexes, $speed);
  } else {
    $pace = $_POST['pace'];
    ...
    $sql_stats = "UPDATE outfield_player_stats SET rating=?, pace=?, shooting=?, passing=?, dribbling=?, defending=?, physical=? WHERE player_id=$id";
    $stmt_stats = mysqli_prepare($conn, $sql_stats);
    mysqli_stmt_bind_param($stmt_stats, 'iiiiiii', $rating, $pace, $shooting, $passing, $dribbling, $defending, $physical);
  }
...
<aside class="relative bg-sidebar h-screen w-64 hidden sm:block shadow-xl">
      ...
      <form action="edit.php" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div class="space-y-1">
            ...
            <select name="position" id="e_position" class="w-full rounded-lg border-gray-200 bg-gray-200 p-3 text-sm">
              <option disabled selected>Select</option>
        ...
        <fieldset class="space-y-4" id="e_outfield_player" style="display: none;">
          <div class="my-3">
        ...
        <fieldset class="space-y-4" id="e_goolkeeper_form" style="display: none;">
          <div class="mt-3">
  ...
  <script>
    const position = document.getElementById("e_position");
    const outfieldForm = document.getElementById("e_outfield_player");
    const goolkeeperForm = document.getElementById("e_goolkeeper_form");
